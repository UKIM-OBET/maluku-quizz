...
<div class="flex items-center justify-between flex-wrap gap-3 mb-5">
    <h2 class="m-0 text-[#0B6B63] font-bold">Soal Kuis: {{ $quiz->title }}</h2>
    <div class="flex gap-2">
        ...
        <a class="inline-flex items-center justify-center px-6 py-3 bg-[#FF6B35] text-white rounded-full font-bold cursor-pointer no-underline transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg" href="/teacher/quizzes/{{ $quiz->id }}/questions/create">Tambah Soal</a>
    </div>
</div>

@if ($quiz->questions->isEmpty())
    <p>Belum ada soal untuk kuis ini. Tambahkan beberapa soal agar murid dapat mengerjakan.</p>
@else
        ...
        <table class="w-full border-collapse min-w-96 bg-white rounded-2xl overflow-hidden border border-[#0B6B63] shadow-md">
            <thead>
                <tr class="bg-gradient-to-r from-[#0B6B63] to-[#2F9E96] text-white">
                    ...
                    <th class="text-left p-4 font-bold uppercase text-sm" style="letter-spacing: 0.05em;">Jawaban Benar</th>
